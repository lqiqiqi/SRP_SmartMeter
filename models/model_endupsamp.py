...
class Net(torch.nn.Module, BaseModel):
    def __init__(self, config):
        ...
        self.first_part = ConvBlock(self.config.num_channels, self.config.d, self.config.k, 1, 3, activation='lrelu', norm=None)
        ...
        for _ in range(int(self.config.m)):
            self.layers.append(ResnetBlock(self.config.s, 5, 1, 2, activation='lrelu', norm='instance'))
        ...
        if self.config.scale_factor == 10:
            self.upsample = nn.Sequential(*[
                nn.Conv1d(in_channels=self.config.d, out_channels=10, kernel_size=5, stride=1, padding=2, bias=False),
            ...
            self.upsample = nn.Sequential(*[
                nn.Conv1d(in_channels=self.config.d, out_channels=self.config.scale_factor, kernel_size=5, stride=1, padding=2, bias=False),
